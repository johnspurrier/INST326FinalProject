...
def get_min_payment(principal, annual_rate, years=30, payments_per_year=12):
    ...
    power = (1+rate)** num_payments
    A = (principal*rate*power)/(power-1)
    return math.ceil(A)
    
def interest_due(balance, annual_rate, payments_per_year=12):
...
def remaining_payments(balance, annual_rate, target, payments_per_year=12):
...
def main(principal, annual_rate, years=30, payments_per_year=12, target=None):
    """computes the minimum payment, displays the minimum payment to the user, sets the target payment to the minimum payment if the user's target payment is equal to None, prints a statement if the user's target payment is lower than the minimum payment
    ...
    if target < min:
        ...
        remain = remaining_payments(principal, annual_rate, target, payments_per_year)
        print(f'if you make payments of {target} you will pay off the mortgage in {remain} payments')
