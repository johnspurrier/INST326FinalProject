...
class HumanPlayer(Player):
    """Child class of Player that represents a real person playing hangman.
    
    Attributes:
        name (str): the player's name, inherited from the parent class
    """
    def turn(self, gamestate):
        """Take a turn.
        
        Args:
            gamestate (Gamestate): a snapshot of the current state of the game
            
        Returns:
            guess (str): the player's guess (a letter or a word)
        """
        print(gamestate)
        guess = input(self.name+ ", guess a letter or type a word to",
                      " solve the puzzle:")
        return guess
    
class ComputerPlayer(Player):
    """Child class of Player that represents a computer hangman player.
    
    Attributes:
        name (str): name of computer player
        vocab (list of str): words the computer player can use when guessing
    """
    ...
    def turn(self, gamestate):
        ...
        Returns:
            guess1 (str/char): single letter guess. Is returned until five
            bad_guesses have been made
            
            guess2 (str): whole word guess from the player's vocab list. Is
            returned after five bad_guesses have been made
        """
        count = 0
        for v in self.vocab:
            if gamestate.match(v):
                guess2 = v
        guess1 = choice(list(gamestate.unguessed))
        count += 1
        if len(gamestate.bad_guesses) > 5:
            return guess2
        else:
            return guess1
